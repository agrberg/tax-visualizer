...
| `section.ts`        | `Section` — a region of rows (the face or Schedule D) queried for a line's amount by id (`amountForId`) or label (`amountForLabel` / `amountForLabelInSegment` / `amountForLabelNear`); owns the lookup logic                                          |
...
| `form1040.ts`       | `Form1040` — encapsulates a parsed 1040: private rows, queried by id/label (`amountForId`/`amountForLabel`/`amountForLabelInSegment`) + a `scheduleD` `Section` + detected `filingStatus`/`taxYear`; owns page classification + `haveEverythingNeeded` |
