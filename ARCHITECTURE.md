...
read _in the same segment_ (`amountForLabelInSegment`, so a shared `3a`/`3b` or
...
| `import/section.ts`           | `Section` of rows (face or Schedule D) queried for a line's amount by id (`amountForId`) or label (`amountForLabel` / `amountForLabelInSegment` / `amountForLabelNear`)               |
