...
const niitOf = (r: ReturnType<typeof calculateTax>) => r.federal.surcharges.find((s) => s.key === 'niit')!;
...
describe('per-source and overall aggregation', () => {
  it('per-source tax sums to total tax and the weighted rate is consistent', () => {
    const r = calculateTax(input({ wages: 150000, interest: 5000, qualifiedDividends: 20000, longTermGains: 10000 }));
...
describe('marginal cost of the next dollar', () => {
  const rate = (scenarios: MarginalScenario[], key: MarginalScenario['key']) => scenarios.find((s) => s.key === key)!;
