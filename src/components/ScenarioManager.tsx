...
import { normalizeName, saveScenario, removeScenario, renameScenario, type Scenarios } from '@/scenarios';
 ...
 */
export function ScenarioManager({ input, selectedName, onSelectedNameChange, onLoad }: ScenarioManagerProps) {
  ...
  const renameValid = dialog?.kind === 'rename' && renameNormalized !== null && renameNormalized !== dialog.oldName;
  // Re-assert `!== null` so the compiler narrows the index access instead of a `?? ''` fallback.
  const renameCollision = renameValid && renameNormalized !== null && scenarios[renameNormalized] !== undefined;
  ...
  return (
    <>
      ...
      <Modal open={dialog !== null} onClose={closeDialog} labelledBy="scenario-dialog-title" className="max-w-sm">
        {dialog && (
          <div className="space-y-4">
            ...
            {dialog.kind === 'update' && (
              <p className="text-sm text-muted-foreground">Replace “{dialog.name}” with the current inputs?</p>
            )}
            {dialog.kind === 'delete' && (
              <p className="text-sm text-muted-foreground">Delete “{dialog.name}”? This can’t be undone.</p>
            )}
            {dialog.kind === 'rename' && (
              <div className="space-y-1.5">
                ...
                {renameCollision && (
                  <p className="text-xs text-amber-700">
                    A scenario named “{renameNormalized}” already exists — renaming will overwrite it.
                  </p>
                )}
              </div>
            )}
