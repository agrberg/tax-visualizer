...
export function ImportReturn({ current, onApply }: ImportReturnProps) {
  ...
  // filing status/year), computed live off the current draft rather than a value frozen at
  // parse time — so it can't go stale as the user edits filing status, year, or the amount
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <DetectedNote
                ...
                provenance={
                  // Derive live from the current draft (via deductionIsItemized / the null
                  // check) so editing the control in the modal never leaves a stale note. Guard
                  // the interpolation so a detected field with no provenance string doesn't
                  // render "undefined — ...".
                  review.provenance.deduction &&
                  (review.draft.deduction === null
                    ? `${review.provenance.deduction} — using the standard deduction`
                    : deductionIsItemized
                      ? `${review.provenance.deduction} (itemized)`
                      : review.provenance.deduction)
                }
