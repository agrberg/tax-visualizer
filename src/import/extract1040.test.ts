...
describe('extract1040Fields — line 12 deduction', () => {
  ...
  it('imports a line 12 above the standard as a custom (itemized) deduction', () => {
    const { fields, provenance } = extract1040Fields(with12('28,500'))
    expect(fields.deduction).toBe(28500)
    expect(provenance.deduction).toBe('1040 line 12 (itemized)')
  })

  it('imports a line 12 below the standard as custom, without the "(itemized)" tag', () => {
    // 10,000 < the 2025 single standard (15,750): a custom amount, but not "itemized" (which
    // only applies above the standard). Doesn't occur on a rational return, but the branch exists.
  ...
  it('imports a custom deduction from line 12e (2025+ layout)', () => {
    ...
    expect(provenance.deduction).toBe('1040 line 12e (itemized)')
  })
  ...
  it('imports line 12 as custom when the year/filing status are unknown (can\'t compare)', () => {
    ...
    expect(provenance.deduction).toBe('1040 line 12') // no "(itemized)" — couldn't compare
  })
})
