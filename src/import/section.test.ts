...
describe('Section.amountForLabelInSegment', () => {
  it('reads the amount right after the label on a single-field row', () => {
    ...
    expect(s.amountForLabelInSegment('taxable interest', ['3a', '3b', '4b'], '2b')).toEqual({
      value: 2100,
      lineId: '2b',
    });
  });
  ...
  it("reads each field's own amount on a shared 3a/3b row (no neighbor bleed)", () => {
    ...
    expect(s.amountForLabelInSegment('qualified dividends', ['3a', '3b', '4b'], '3a')?.value).toBe(1000);
    expect(s.amountForLabelInSegment('ordinary dividends', ['3a', '3b', '4b'], '3b')?.value).toBe(3000);
  });
  ...
  it('returns null when the labeled line has no amount in its segment', () => {
    ...
    expect(s.amountForLabelInSegment('standard deduction or itemized deductions', [], '9')).toBeNull();
  });
  ...
  it('reads a 1-2 digit amount without mistaking a reprinted id for it', () => {
    ...
    expect(s.amountForLabelInSegment('capital gain or (loss)', [])?.value).toBe(7);
  });
  ...
  it('reads the deduction on a merged 12a/12b/12c row when ownId is omitted', () => {
    ...
    expect(s.amountForLabelInSegment('standard deduction or itemized deductions', ['12a', '12b', '12c'])?.value).toBe(
      13850,
    );
  });
  ...
  it('returns null when no row contains the label', () => {
    ...
    expect(s.amountForLabelInSegment('pensions and annuities', [])).toBeNull();
  });
});
