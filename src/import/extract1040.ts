...
function readInterest(form: Form1040, draft: Draft): void {
  ...
  if (byId === null) return;
  const byLabel = form.amountForLabelInSegment('taxable interest', SHARED_LINE_IDS, id);
...
function readDividends(form: Form1040, draft: Draft): void {
  ...
  if (qualified !== null) {
    const byLabel = form.amountForLabelInSegment('qualified dividends', SHARED_LINE_IDS, qualifiedId);
  ...
  if (ordinary !== null) {
    const byLabel = form.amountForLabelInSegment('ordinary dividends', SHARED_LINE_IDS, ordinaryId);
...
function readWages(form: Form1040, draft: Draft, year: number | null): void {
  ...
  const byLabel = form.amountForLabel('wages, salaries, tips');
  if (byLabel !== null) {
 ...
 *  the pensions line and the one after it (see `amountForLabelNear`). Returns the amount or null. */
function readPensions(form: Form1040, draft: Draft, year: number | null): number | null {
  const byLabel = form.amountForLabelNear('pensions and annuities', 'taxable amount');
...
function readCapitalGains(form: Form1040, draft: Draft, year: number | null): void {
  ...
  const byLabel = form.amountForLabelInSegment('capital gain or (loss)', SHARED_LINE_IDS, id ?? undefined);
...
function readDeduction(form: Form1040, draft: Draft, year: number | null): void {
  ...
  const byLabel = form.amountForLabelInSegment(
